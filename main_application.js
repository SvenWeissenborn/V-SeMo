...
canvas.on('mouse:move', function (o) {
    ...
    if(isVectorPointDragged === true) {
        ...
        let vectorPointParentID = vectorPoint.parentSector[0];
        ...
        let closestEdgeOfPointParameters = getClosestEdgeOfPointParameters(pointer, vectorPointParentID);
...
async function autoSetSectorsAlongGeodesic(chosenGeodesicToSetSectors) {
    ...
    if (typeof chosenGeodesicToSetSectors === 'undefined') {
        ...
        if ( lines[chosenGeodesicToSetSectors].length > 0) {
            ...
            let staticSectorPos = new fabric.Point(sectors[staticSector].trapez.left, sectors[staticSector].trapez.top)

            let sectorArrayToAlignIDAnglePos = [
                [
                    staticSector,
                    sectors[staticSector].trapez.angle,
                    staticSectorPos
                ]
            ]
...
function drawVector(vectorPointCoords, vectorHeadCoords, vectorHeadOrientation, vectorParentID, vectorGlobalID, vectorLocalID, type, pushHistoryToSet) {
    ...
    if(type === 'vector') {
        ...
        vectorHead.on('moving', function(o) {
            ...
            let alpha = toRadians(vectorAngle) - toRadians(vectorHeadAngleOnMouseDown);
...
window.addEventListener('keydown', function(event) {
    ...
    /*let sectorArrayToSnap = [[1,2],[4,1],[3,4], [6, 3]]


        // Function to animate a single sector and return a promise
        function animateSector(sector) {
            return new Promise((resolve) => {
                animateRotation(sector[0], sector[1]);
                // Assuming animatedSnapInitialSectorToTargetSector has a callback or some way to determine when the animation is complete
                // In this example, we use a setTimeout to simulate the completion of the animation after 1000ms.
                setTimeout(() => {
        ...
        // Loop through each sector and animate them sequentially using async/await
        async function animateAllSectors() {
            for (let ii = 0; ii < sectorArrayToSnap.length; ii++) {
                await animateSector(sectorArrayToSnap[ii]);
            }
        }

        animateAllSectors(); // Start the sequential animation*/
    }
});
