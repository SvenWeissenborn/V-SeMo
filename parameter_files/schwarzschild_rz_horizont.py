...
#Kameraeinstellungen
startZoom = 0.6
startViewportTransform_4 = -400
startViewportTransform_5 = 450
...
def main():
    ...
    file = io.open("schwarzschild_rz_horizont.js",'w')

    file.write( "/*" +"\n"
                "------Parameter-------" + "\n"
                "radius: " + str(schwarzschildRadius) + "\n"
    ...
    for zeile in range(0, nRowsInModel):
        ...
        for ii in range(0, nColumnsInModel):
            ...
            timeEdgeLeft = 2 * Delta

            timeEdgeRight = 2 * Delta

            tauZeile = - schwarzschildRadius + zeile * 2 * Delta

            spaceEdgeBottom = 2 * Delta * math.pow((1 - 3/2 * (tauZeile - Delta) / schwarzschildRadius), (- 1/3))

            spaceEdgeTop = 2 * Delta * math.pow((1 - 3/2 * (tauZeile + Delta) / schwarzschildRadius), (- 1/3))
            ...
            #Utes Hoehenberechnung fuehrt zum gleichen Aussehen der Trapeze
            #sectorHeight = math.sqrt(math.pow((spaceEdgeTop - spaceEdgeBottom), 2) / 4 + 4 * Delta*Delta)
